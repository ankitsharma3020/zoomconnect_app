  ...
  const handleVerifyOTP = async () => {
   ...
   const handleReSendOtp = async () => {
     ...
     setIsResendDisabled(true)
      ...
      setLoading(true);
      try {
        const response = await Mobilelogin({ mobile: data }).unwrap();
         
        
        if (response && (response.success || response.token || response.status === 'success')) {
          // Navigates to OTP with the target mobile number
             ToastAndroid.show(response?.message || 'OTP sent successfully', ToastAndroid.SHORT);
        } else {
